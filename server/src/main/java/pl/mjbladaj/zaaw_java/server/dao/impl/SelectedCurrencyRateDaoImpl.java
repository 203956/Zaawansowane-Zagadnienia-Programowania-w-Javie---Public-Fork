...
public class SelectedCurrencyRateDaoImpl implements SelectedCurrencyRateDao {
    ...
    public UniversalRate getRate(String fromCurrency, String toCurrency) throws EntityNotFoundException {
        ResponseEntity<FreeCurrenciesComRate> response = restTemplate
                .getForEntity( getUrl(fromCurrency, toCurrency), FreeCurrenciesComRate.class);
        FreeCurrenciesComRate freeCurrenciesComRate = response.getBody();
        return UniversalRateConverter.getCurrencyRate(freeCurrenciesComRate, fromCurrency, toCurrency);
    }
